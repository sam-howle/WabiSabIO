...
| Function | Description |
  ...
  | [`randomize_coordinate_within_square(x, y, radius)`](#coordinate-randomization) | Same as `randomize_coordinate_within_range()`, but uses only uses one `radius` value, as X and Y distance from center is equal for square-shaped UI elements |
